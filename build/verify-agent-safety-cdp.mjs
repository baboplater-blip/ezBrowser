...
const results = []
// ── 시나리오 간 상태에 관한 규칙 (2026-09-07, 임무 11·12 실측) ───────────────
//
// 이 하네스의 14개 시나리오는 **하나의 페이지·하나의 CDP 세션**을 공유한다. 이건 우연이 아니라
// 설계다 — 앱을 14번 띄우지 않으려는 것이고, 실제로 전체가 1분 남짓에 끝난다.
//
// 그래서 **시나리오 간 의존성이 존재한다**:
//   · A1 이 `obs` 에 담은 관찰 결과를 **A2·A3 가 그대로 재사용**한다.
//     ref 레지스트리는 페이지 전역(window)에 살아 있으므로 **리로드하면 ref 가 전부 무효**가 된다.
//
// ⚠ **시나리오 사이에 페이지를 리로드하지 말 것.** 2026-09-07 에 "공통 resetPage()" 로
//   매 시나리오 앞에서 리로드해 봤다가 **A2·A3·A12·A13 이 한꺼번에 깨졌다**(A2·A3 는 위 이유,
//   A12·A13 은 원인 미확정). 격리는 좋은 원칙이지만 **이 스위트에는 맞지 않는다** — 되돌렸다.
//
// ✅ 올바른 방법: **정리는 시나리오마다, 자기가 방해받을 것만.** A14 가 그 예다 —
//   시작할 때 앞 시나리오가 주입한 DOM(#ov, .upload-dropzone, #wrap, .menu-dropdown, #zone)을
//   제거하고 스크롤을 되돌린다. 새 시나리오를 추가할 때 앞의 잔재가 방해가 되면 같은 방식으로
//   자기 앞에서만 치울 것.
//
// (남은 잔재가 대상 버튼을 덮으면 **제품이 멀쩡한데도** 실제 클릭이 합성 폴백으로 떨어져
//  간헐 실패한다 — A14 가 실제로 그렇게 약 1/5 확률로 실패했다.)
