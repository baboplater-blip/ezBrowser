...
**권장: 통합 러너 한 줄.** 아래 개별 명령을 손으로 돌리는 대신

```powershell
npm run verify        # 게이트 0 + 게이트 1 스모크 16종 (약 40초, 종료코드로 판정)
npm run verify:full   # 게이트 0~5 전 하네스 (20~40분, 네트워크 사용)
```

러너는 실패를 삼키지 않는다 — 하나라도 실패하면 **비0으로 종료**하고 실패 로그 꼬리를 출력한다.
패키징 전 실행 중인 앱이 exe 를 잠그면 `BLOCKED` 로 **PID 를 지목해** 알려준다(러너가 임의로 죽이지 않음).
개별 명령이 필요할 때만 아래를 쓴다.
...
- **라운드 완료 = `npm run verify` 통과 + 결과 요약을 보고서에 기록.**
  (게이트 0 + 스모크 16종, **약 30초 · 창 1개**. status.md 기록은 자동.)
- **`npm run verify:full`(11단계·약 10분)은 사용자가 자리를 비울 때·출시 전에만.**
  실제 창을 여러 번 띄우고 50탭 스트레스·성능 측정을 포함해 **화면을 10분간 점유**한다.
  - 부분 실패가 있으면 **그 항목을 "못 한 것"으로 명시**하고 넘어간다 — 숨기지 않는다.
  - 예전 규칙("게이트 0 + 건드린 영역의 게이트")은 **무엇을 돌릴지 사람이 판단**해야 해서
    빠뜨리기 쉬웠다(묶음 YTDLP-1 이 게이트 0만 돌고 끝난 사례). 이제 목록은 러너가 갖는다.
- **출시(v1.0) = 게이트 0~7 전부 + goal.md Definition of Done 전 항목.**
- 실패 항목은 status.md "알려진 위험"에 적고 plan.md에 작업으로 승격한다.
- **검증 실행 기록은 `verify-all` 이 자동으로 남긴다** — `npm run verify:full` 이 status.md 의
  `<!-- verify-all:begin -->` 섹션(최신 실행 표 + 최근 10회)을 덮어쓴다. **그 섹션은 손대지 말 것.**
  (사람이 표를 옮겨 적는 한 그 단계는 언젠가 생략된다 — 그래서 자동화했다.)
- **해석·판단은 사람이 쓴다** — status.md "최근 라운드 로그"에 무엇을 왜 했고 무엇이 남았는지.
  숫자는 자동 섹션이, 의미는 사람이 담당한다. 기록 없는 검증은 안 한 것으로 친다.
