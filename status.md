...
**`npm run verify` — 6/6 PASS · 1m 12s** (2026-09-07 07:38)
...
- 2026-09-07 06:19 · `quick` · **4/4 PASS** · 33s
- 2026-09-07 05:52 · `quick` · **4/4 PASS** · 32s
- 2026-09-07 05:37 · `full` · **16/16 PASS** · 12m 18s
...
- 2026-09-07 04:57 · `quick` · **4/4 PASS** · 32s
...
### 2026-09-07 — auto-dev 임무 15·16·17: 예산 단일 출처 · 사용자 화면 3종 실측 대조
- **임무 15**: 가벼움 예산 상수를 **[`app/shared/perf-budget.json`](app/shared/perf-budget.json) 단일 출처**로(값마다 근거 주석). `build/lib/budget.mjs` 로더가 필수 키 검증. 하네스·앱·문서가 같은 값을 본다.
- **임무 16**: `browser://memory` **표시값 실측 대조 하네스 M1~M6**(탭 3분할 정합·프로세스 수 vs OS·슬립 표시가 설정을 따라오는지). **6/6 PASS**. 탭 카드 라벨 `활성` → **`깨어 있음`**(브라우저에서 "활성 탭"은 포커스 탭으로 읽혀 오해).
- **임무 17**: `browser://settings`·`welcome` **양방향 대조 하네스 S1~S5·W1~W3** — 표시(설정→UI)·반영(UI→설정)·구독(밖에서 바꾸면 따라오는지)·15개 카테고리 렌더·온보딩 5단계·완료 기록. **8/8 PASS**(제품 결함 없음).
- **두 하네스 모두 "제 하네스가 먼저 틀렸다"**: 부분일치 스크래핑(279를 프로세스 수로), 앞 단계 상태 의존(마지막 카테고리에 머문 채 조회). **테스트가 앞 단계 상태에 기대면 조용히 틀린다.**

### 2026-09-07 — auto-dev 임무 13·14: 검증 기록 자동화 · 메모리 페이지 기준선 연동
- **임무 13**: `verify-all` 이 status.md 의 `<!-- verify-all:begin -->` 섹션을 **자동 갱신**(최신 실행 표 + 최근 10회, 멱등 치환). 손으로 쓰는 라운드 로그와 분리 — **숫자는 도구가, 해석은 사람이**.
- **검증 실행 정책 B(사용자 선택)**: 라운드 종결은 **`npm run verify`(약 30초·창 1개)**, `verify:full`(11단계·약 10분·실제 창 다수)은 **자리를 비울 때·출시 전에만**. 하네스는 격리 프로필만 쓰므로 사용자 데이터는 무접촉.
- **임무 14**: 메모리 페이지의 기준선 **하드코딩 제거** → `build/gen-perf-baseline.mjs` 가 실측 이력에서 구운 `perf-baseline.json` 을 빌드 시점에 주입(설치된 앱은 개발 머신의 이력 파일을 읽을 수 없으므로). perf 이력에 **2축(adblock 제외·adblock 비용)** 추가.
- 검증: `browser://memory` CDP 실기 확인 · perf 실측 `{총계 250, adblock 제외 147, adblock 비용 103}` · 종결 게이트 4/4 PASS.

### 2026-09-07 — auto-dev 임무 12: 시나리오 격리 시도 → 되돌림 (실패한 실험의 기록)
- **시도**: 공통 `resetPage()`(매 시나리오 앞 페이지 리로드) → **A2·A3·A12·A13 붕괴**. 원인 확인: A1 의 관찰 결과 `obs` 를 A2·A3 가 재사용하고 **ref 레지스트리가 페이지 전역**이라 리로드하면 무효.
- **되돌림**: 자급자족으로 바꾸면 "ref 지속성" 회귀 감지력을 잃는다 — 격리가 이 스위트 설계와 안 맞는다. 14/14 복귀.
- **대신 규칙을 하네스 헤더에 명문화**: ⚠ 시나리오 사이 리로드 금지 / ✅ 정리는 시나리오마다 자기가 방해받을 것만(A14 방식).
