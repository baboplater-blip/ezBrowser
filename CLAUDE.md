...
**라운드 중에는 `npm run verify`** (게이트 0 + 스모크 16종, 약 30초).
**라운드를 닫을 때는 `npm run verify:full` — 이것이 종결 절차다** (전 하네스 11단계, **실측 약 10분**).

> **종결 규칙 (2026-09-07 확립)**: 라운드 완료를 선언하기 전에 `npm run verify:full` 을 돌리고,
> 출력 끝의 **보고서용 요약 블록**을 그 라운드의 귀환 보고서에 붙인다.
> **status.md 기록은 자동이다** — `--full` 실행이 `<!-- verify-all:begin -->` 섹션(최신 실행 + 최근 10회)을
> 덮어쓴다(그 섹션은 손대지 말 것). 사람은 라운드 로그에 **해석·판단**만 쓴다.
> 기준선은 **11/11 PASS · 약 10분**(2026-09-07 실측). 10분이면 라운드마다 돌릴 만큼 싸다 —
